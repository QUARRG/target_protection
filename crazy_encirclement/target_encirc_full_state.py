...
class CircleDistortion(Node):
    def __init__(self,swarm=None):
        ...
        """
        super().__init__('target_encirclement')
        self.swarm = swarm
        self.info = self.get_logger().info
        self.info('Target encirclement node has been started.')
...
def main():
    swarm = Crazyswarm() #this parameter is necessary to arm the new cfs. Don't pass this parameter if running old cf
    if not rclpy.ok():
        rclpy.init()
